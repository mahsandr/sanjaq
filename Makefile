...
test: bin/gotestsum ## Run tests
	...
	bin/gotestsum --no-summary=skipped --junitfile ${BUILD_DIR}/test_results/${TEST_REPORT}/${TEST_REPORT_NAME} --format ${TEST_FORMAT} -- $(filter-out -v,${GOARGS}) -coverprofile=coverage.out -race -parallel 1 -p 1 $(if ${TEST_PKGS},${TEST_PKGS},./...)
